...
class PlanningGraph():
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            count = 0
            for states_nodes in self.s_levels:
                # Get all literals for given node
                literals = set([s_node.literal for s_node in states_nodes])
                if goal in literals:
                    break
                else:
                    # Count ;evel if the goal in the literals
                    count += 1  
